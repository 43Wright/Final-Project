-------
Group Members: Gabriel Wright, Brennan Sauser
Project: Student Spending
Class: DS 202
output: github_document
-------

```{r setup, include = FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(readr)
library(tidyverse)
library(dplyr)
```

#Analysis on Student Spending Habits
## Introduction:
Goal of the project is to explore the dataset to be able to visualize student spending habits

Exploring the following questions/prompts:
1) What percentage is their monthly income is used for other expenses?

2) Does different majors affect student's income and expenses?

3) Which category has the highest expenses?

4) Which Majors have the highest Tuitions

5) School Expenses based on Major

6) Does a student's gender have any indication of their major (what majors each gender typically go into)

7) Difference in student's expenses based on gender

8) student spending based on year

9) Major distribution by year and gender

10) Correlations of each variable/attribute

## Data Structure:
Dataset: "https://www.kaggle.com/datasets/sumanthnimmagadda/student-spending-dataset/data"
Variables Inlcuded in the dataset:
1) Age:
2) Gender:
3) Year in School:
4) Major:
5) Montly Income: of the student (in dollars)
6) Financial Aid: financial aid recieved by the student (in dollars)
7) Tuition: Expenses for tuition (in dollars)
8) Housing: Expenses for Housing (in dollars)
9) Food: Expenses for food (in dollars)
10) Transportation: Expenses for transportation
11) Book and Supplies: Expenses for books and supplies
12) Entertainment: Expenses for Entertainment (in dollars)
13) Personal Care: Expenses for personal care items (in dollars)
14) Technology: Expenses for technology (in dollars)
15) Health & Wellnesss: Expenses for health and wellness (in dollars)
16) Miscellaneous: Miscellaneous expenses (in dollars)
17) Preferred Payment Method: Method of payment (Cash, Credit/Debit card, Moblie Payment App)

## Cleaning
``` {r}
spending <- read_csv('student_spending.csv')
spending
```

``` {r}
#Checking for null values
sum(is.na(spending))
#No null or missing values

#Summary of data
summary(spending)

#Checking dimensions of the data
dim(spending)
#There are 1000 rows and 18 columns

#Removing Preferred Payment Method column since it is not relevant to the questions we want to answer
spending %>% select(-preferred_payment_method)
```
## Results/Questions we want to answer
``` {r}
### Percentage of monthly income used for expenses other than school costs

#spending %>% ggplot(aes())
```

``` {r}
### Distribution of majors based on their income and expenses

#spending %>% ggplot(aes())
```

``` {r}
### Which category produces the highest expenses
#total_trans <- sum(spending['transportation'])
#total_books <- sum(spending['books_supplies'])
#total_enter <- sum(spending['entertainment'])
#total_tut <- sum(spending['tuition'])
#total_food <- sum(spending['food'])
#total_person <- sum(spending['personal_care'])
#total_tech <- sum(spending['technology'])
#total_health <- sum(spending['health_wellness'])
#total_misc <- sum(spending['miscellaneous'])
#spending %>% ggplot(aes(x = c(total_trans, total_books))) + geom_col()
```

``` {r}
### Which Majors have the highest tuition 

#spending %>% ggplot(aes())
```

``` {r}
### School Expenses based on major

#spending %>% ggplot(aes())
```

``` {r}
### Distribution of Gender and Majors


#spending %>% ggplot(aes(x = group_by(gender), y = major)) + geom_bar() 
```

``` {r}
### Difference in students expenses and income based on gender

#Displays each genders average income
spending %>% group_by(gender) %>%
  summarize(m = mean(monthly_income),
            n = n()) %>% ungroup()

#spending %>% ggplot(aes())
```

``` {r}
### Student spending based on year in school

#spending %>% ggplot(aes())
```

``` {r}
### Distribution of total different expenses 

#spending %>% ggplot(aes(x = c('transportation', 'book_supplies', 'entertainment')) + geom_point()
```
