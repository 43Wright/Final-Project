...
# Psycology students tend to have the most expenses, with computer science students with the second most expenses
```
...
#Biology students on average tends to have the most financial aid
```
...
#Male students on average tend to receive more financial aid
```

``` {r}
## Student financial aid based on year
spending %>% group_by(year_in_school) %>% ggplot(aes(x = year_in_school, y = mean(financial_aid))) + geom_col(fill = 'black') + labs(
        title = "Average Student Financial Aid based on year",
        x = "Year",
        y = "Financial Aid ($)",
    )
#Financial aid is very similar for each year, not much significant in their differences
```
...
#on average financial aid covers for about 11% of an average students tuition
```
